...
def _find_min_index(data: list, mark: int) -> int:
    """
...
def _partition(data, pivot):
    """
    Three way partition the data into smaller, equal and greater lists, in relationship to the pivot
    ...
    less, equal, greater = [], [], []
    for element in data:
        if element.sum > pivot:
            less.append(element)
        elif element.sum < pivot:
